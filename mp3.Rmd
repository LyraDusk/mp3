...
get_height <- function(trail_name) {
  ...
  summarize(max_h = max(ELEV_M), min_h = min(ELEV_M), diff_h = max_h - min_h,length = max(len)) #%>%
   # mutate(difficulty = sqrt(difficulty))
   
  gsub('.{0,2}$', '', get_height::diff_h)
   
  get_height %>%
    summarize(difficulty = sqrt(as.numeric(diff_h) * 2 * length))
}
